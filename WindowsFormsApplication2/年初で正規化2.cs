...
{
    public partial class 年初で正規化2 : Form
    {
        public 年初で正規化2()
        {
            InitializeComponent();
        ...
        private void Get_file_List_Button(object sender, EventArgs e)
        {
            ...
            //ここから正規化計算
            //
            out_data_file = compute_normalize( in_data_file, num_stock, days2 );
            max_date = compute_normalize2(in_data_file, num_stock, days2, 1);
            min_date = compute_normalize2(in_data_file, num_stock, days2, 2);
            max_value2 = compute_normalize2(in_data_file, num_stock, days2, 3);
            min_value2 = compute_normalize2(in_data_file, num_stock, days2, 4);
        ...
        public string[,] compute_normalize( string[,] in_data_file, int num_stock, int days2 )
        {
            int stock_num = 0;
                ...
                for (days3 = 0; days3 < days2; days3++)
                {
                    if (stock_num >= 1)
                    {
                        if (days3 >= 2)
                        {
                            ...
                            {
                                in_data_file2[days3, stock_num] = double.Parse(in_data_file[days3, stock_num]);
                                ...
                                if (in_data_file2[2, stock_num] != 0)
                                {
                                    ...
                                    //年初の値で正規化計算
                                    //
                                    out_data_file2[days3, stock_num] = in_data_file2[days3, stock_num] / in_data_file2[2, stock_num] * 100;
        ...
        public string[] compute_normalize2(string[,] in_data_file, int num_stock, int days2, int output)
        {
            int stock_num = 0;
        ...
        public void write_file(string dname, string[,] out_data_file, string[] max_date, string[] min_date, string[] max_value2, string[] min_value2, int days2, int num_stock)
        {
            ...
            System.IO.File.Delete(@dname + "_" + "_Norm.csv");

            Encoding sjisEnc = Encoding.GetEncoding("Shift_JIS");
            System.IO.StreamWriter writer = new System.IO.StreamWriter(@dname + "_" + "_Norm.csv", true, sjisEnc);
